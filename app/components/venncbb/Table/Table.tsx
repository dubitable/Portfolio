...
import type { SelectedColumn, OrderBy } from "../../../routes/projects/venncbb";
import {
  entries,
  keys,
  selectableColumn,
...
import ArrowIcon from "../../../components/icons/venncbb/ArrowIcon";
    ...
    <div className="flex w-[60rem] overflow-scroll h-screen py-[3rem] justify-center flex-col gap-2">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y-2 divide-gray-700">
          <thead className=" ltr:text-left rtl:text-right bg-gray-900">
            <tr className="*:font-medium *:first:sticky *:first:left-0 *:first:bg-gray-900">
              {keys(teams[0])
                .slice(SKIP_COLS)
                .map((key, index) => (
                  <th
          ...
          <tbody className="divide-y divide-gray-700">
            {paginate.slice(teams).map((team, teamIndex) => (
              <tr
                className="*:first:sticky *:first:left-0 *:first:bg-[#242424]"
                key={teamIndex}
              >
                {entries(team)
                  .slice(SKIP_COLS)
